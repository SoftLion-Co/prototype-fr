...
const Project = async ({ params }: { params: any }) => {
  ...
  const projectDescriptionSection = {
    ...
    imgSrc: response.pictures.find((picture) => picture.position === 0) ? response.pictures.find((picture) => picture.position === 0)?.url || "" : ""
  };
  ...
  const solutionSection = {
    ...
    images: [
      response.pictures.find((picture) => picture.position === 0) ? response.pictures.find((picture) => picture.position === 1)?.url || "" : "",
		response.pictures.find((picture) => picture.position === 0) ? response.pictures.find((picture) => picture.position === 2)?.url || "" : "",
		response.pictures.find((picture) => picture.position === 0) ? response.pictures.find((picture) => picture.position === 3)?.url || "" : ""
    ],
  };
  const resultSection = {
    paragraphs: [
      response.resultFirstParagraph,
      response.resultSecondParagraph,
      response.resultThirdParagraph
    ],
    imgUrl: response.pictures.find((picture) => picture.position === 0) ? response.pictures.find((picture) => picture.position === 4)?.url || "" : ""
  };
